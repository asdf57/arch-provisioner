...
create_elevated_rootfs() {
    ...
    # # Utilize unshare to create separate pid ns (prevents any orphaned pids)
    # sudo env \
    ...
    #     /sbin/tini -- /homelab/scripts/init_nix.sh
    ...
    # sudo unshare --pid --mount-proc --fork --root="$chroot_dir" /bin/sh /homelab/scripts/init_nix.sh
}
