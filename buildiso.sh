...
HOST_SSH_KEY_PATH="$HOME/.ssh/arch_provisioning_key"
...
limactl copy "$HOST_SSH_KEY_PATH.pub" "${LIMA_INSTANCE}:${VM_SSH_KEY_PATH}"
...
# Add required packages to packages list for the Arch ISO
limactl shell $LIMA_INSTANCE -- sudo bash -c "echo -e 'openssh\njq\npython\npython-pip\nansible' >> ${VM_ARCHISO_DIR}/releng/packages.x86_64"
...
run_in_chroot sed -i 's/#PasswordAuthentication yes/PasswordAuthentication no/' /archiso/releng/airootfs/etc/ssh/sshd_config
